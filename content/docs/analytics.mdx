...
How much detail you see depends on the site's [collection mode](/en/collection-modes): in `anonymous` mode the tracker sends error **type and count only**. Messages and stack frames arrive in `full` mode, or in `consent` mode after the visitor opts in.
...
A visitor is counted along an identity axis: `stitch` (the default, present in every mode) or `session` (the cross-page `session_id`, which the tracker only carries in `full` mode or in `consent` mode after opt-in). The site's `preferred_identity` sets the default. Unlike the old sessions view, the explorer is **not** empty on an `anonymous`-mode site: it counts `stitch` visitors there. The per-visitor replay, however, needs the `session` identity, so it stays unavailable without consent. See [Visitors and sessions](/en/concepts/sessions) for the full model.
...
- [Collection modes](/en/collection-modes): what the tracker collects per mode, and why the session identity or stack traces may be missing.
