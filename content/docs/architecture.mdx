...
Two spots in the pipeline matter if you want to understand the behavior:

- **Browser → collector.** A few batched `POST`s per page-view, spread over its lifecycle (load, `pagehide` / `visibilitychange`, periodic flush). See [The RUM Beacon](/en/concepts/beacon).
...
All three are operated in the EU with versioned, idempotent migrations applied on deploy.
