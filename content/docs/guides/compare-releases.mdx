...
1. In fastmon, open **Account → API keys** and click **Create key**.
2. Pick the organization the site belongs to and tick `release:write` and `analytics:read`; the release call and the comparison query below need nothing else. Choose an expiry.
3. Copy the value: it starts with `fm_` and is shown once. Store it in your CI's secret store as `FASTMON_API_KEY`.

A pipeline that only posts releases can skip the account page: the **CI/CD** snippet on the application page creates a `release:write` key for that organization in place and writes it into the curl command.

If you ever lose the key, revoke it and create a new one. The old one stops working immediately. See [Authentication](/en/api/authentication) for what a key can and cannot do.
