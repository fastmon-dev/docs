...
Die bloßen Flags von CloudFront werden als Fallback für die CDN-Schicht erkannt: `cdn-cache-hit` → `HIT`, `cdn-cache-miss` → `MISS`, `cdn-cache-refresh` → `REVALIDATED` (ein explizites CDN-Key überschreibt sie weiterhin).

<Callout type="warn" title="fm-fpc ist weg">
...
| Key             | Spalte       | Bedeutung                                        |
| --------------- | ------------ | ------------------------------------------------ |
| `fm-cdn-age`    | `cdn_age`    | Alter des ausgelieferten CDN-Objekts, in Sekunden |
| `fm-origin-age` | `origin_age` | Alter des ausgelieferten Origin-Objekts, in Sekunden |
