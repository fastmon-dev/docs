...
1. **Hochgestufte Skalare** (immer behalten): die acht [Backend-Phasen](#backend-phasen), die zwei [Cache-Status](#cache-status), den [Seitentyp](#seitentyp) und die zwei [Label-Dimensionen](#origin-host-und-logged-in) (`origin_host`, `logged_in`). Sie werden zu dedizierten Spalten.
...
| `db_dur`      | `fm-db`, `db`, `sql`, `rdbms`; dazu `mongodb`, `sqlite`                   | erster mit dur, **+ Summe**  |
| `render_dur`  | `fm-render`, `render`, `wp-template`, `view`, `ssr`                       | erster mit dur               |
| `http_dur`    | `fm-http`, `http`, `fetch`, `api`, `ext`                                  | **Summe**                    |
| `search_dur`  | `fm-search`, `elasticsearch`, `opensearch`, `solr`, `search`              | **Summe**                    |
| `kv_dur`      | `fm-kv`, `redis`, `valkey`, `kv`, `memcache`, `cache`                     | **Summe**                    |
...
**Historie.** `search_dur` und `kv_dur` gibt es seit dem 27. August 2026; davor zählte `redis` in `cache_dur`, und `elasticsearch` zählte nirgends. Seit dem 28. August 2026 ist `cache_dur` in `kv_dur` aufgegangen, und `external_dur` heißt `http_dur` (gleiche Mitglieder). Gespeicherte Werte wurden nicht umgeschrieben und `http_dur` nicht rückwirkend befüllt: Über einen langen Zeitraum macht `kv_dur` an diesem Datum eine Stufe nach oben, und `http_dur` beginnt dort. `cache_dur` und `external_dur` sind aus der API verschwunden; das Detail pro Key vor diesem Datum steht weiterhin in `server_timing_dur`. Notification Rules, die eine der alten Metriken nennen, werden weiter auf der Nachfolgerin ausgewertet; neue Rules nehmen die neuen Namen.

Alle acht Phasen erscheinen auf der Server-Timing-Seite, im Wasserfall der Response-Aufschlüsselung und als Metriken und Spalten im Explorer (die KV-Phase heißt „KV / Cache"; der Full-Page-Cache ist der [Cache-Status](#cache-status) des Origins, eine andere Schicht). Der Pageview-Drawer listet die Phasen verschachtelt: DB, Search, KV / Cache, HTTP und Render unter Backend, Backend unter Origin, jede mit ihrem Anteil an der Zeile, die sie enthält, und die keiner Schicht zugeordnete Zeit als Zeile „Nicht zugeordnet".
...
<Callout type="warn" title="Nur das grobe Flag">
