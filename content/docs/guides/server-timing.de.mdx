...
1. **Hochgestufte Skalare** (immer behalten): die neun [Backend-Phasen](#backend-phasen), die zwei [Cache-Status](#cache-status) und den [Seitentyp](#seitentyp). Sie werden zu dedizierten Spalten.
2. **Tier-2-Katalog** (ohne Cap): ein bekannter Satz von Namen (`mongodb`, `kafka`, `dns`, `cpu`, `auth`, `gc`, `parse`, `session`, `theme`, `compressiontime` sowie die Mitglieder der summierten Phasen unten) landet in den Dauer-/Description-Maps des Pageviews.
...
| `cache_dur`    | `fm-cache`, `cache`, `memcache`, `apcu`                       | **Summe**    |
| `external_dur` | `fm-external`, `http`, `fetch`, `api`, `ext`                  | **Summe**    |
| `search_dur`   | `fm-search`, `elasticsearch`, `opensearch`, `solr`, `search`  | **Summe**    |
| `kv_dur`       | `fm-kv`, `redis`, `valkey`, `kv`                              | **Summe**    |

Phasen, die du nicht sendest, fehlen schlicht (NULLs werden übersprungen, nie als Null gezählt). Die Perzentile zeigen dir z. B., dass `db` dein p95-Problem ist, während `render` flach bleibt. `db_dur` ist rein relational: Zeit in Elasticsearch oder Redis fließt nie hinein.

Die Store-Phasen sind absichtlich getrennt. `search_dur` und `kv_dur` gibt es seit dem 27. August 2026; davor zählte `redis` in `cache_dur`, und `elasticsearch` zählte nirgends. Redis kann Session-Store, Queue oder Primärspeicher sein, sein Wert misst also nur „Zeit in Redis" und landet jetzt in `kv_dur`; `cache_dur` behält die Keys, die eindeutig ein Cache sind. Ist dein Redis *dein* Cache, sende zusätzlich zu `redis` ein `fm-cache` (oder `cache`). Shopifys `edge_cart` ist ganz aus den Summen raus und bleibt für den Drill-down in den Maps. Gespeicherte Werte wurden nicht umgeschrieben; auf einer betroffenen Site macht ein `cache_dur`-Chart über einen langen Zeitraum an diesem Datum deshalb eine Stufe nach unten.

Alle neun Phasen erscheinen auf der Server-Timing-Seite, im Wasserfall der Response-Aufschlüsselung und als Metriken und Spalten im Explorer (die Cache-Phase heißt „Cache"; der Full-Page-Cache ist der [Cache-Status](#cache-status) des Origins, eine andere Schicht). Der Pageview-Drawer listet die Phasen verschachtelt: DB, Cache, Search, KV, External und Render unter Backend, Backend unter Origin, jede mit ihrem Anteil an der Zeile, die sie enthält, und die keiner Schicht zugeordnete Zeit als Zeile „Nicht zugeordnet".
...
Beide Schichten nutzen dasselbe Vokabular. Ein Pageview, der keinen Cache-Status meldet, wird leer gespeichert und erscheint im Dashboard als **Unknown** (in den Share-Metriken pro Status zählt er als `unreported`). Zusammen sind sie der schnellste Blick darauf, ob dein Edge-Cache wirklich greift, direkt neben [TTFB](/de/web-vitals/ttfb).
...
Fastmon akzeptiert `pageType` / `page_type` und das First-Party `fm-pagetype`, schreibt das Label klein und kappt es bei 32 Zeichen. Der Header-Wert gewinnt immer gegen die Body-Class-Erkennung, die du mit [`pagetype_ruleset`](/de/tracker#pagetype-ruleset) aktivieren kannst, und ist damit die maßgebliche Quelle, sobald du ihn serverseitig setzen kannst.
...
- **Serverzeit länger als TTFB ist kein Datenfehler.** Hinter 103 Early Hints ist das erste Byte die vorläufige Antwort der Edge, und die Phasen beschreiben die finale; `edge_dur + backend_dur` muss in `ttfb_final` passen, nicht in `ttfb`. Siehe [TTFB → Early Hints](/de/web-vitals/ttfb#early-hints).
