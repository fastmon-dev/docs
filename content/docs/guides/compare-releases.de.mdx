...
1. Öffne in fastmon **Account → API-Keys** und klicke **Key erstellen**.
2. Wähle die Organisation, zu der die Site gehört, und hake `release:write` und `analytics:read` an; mehr brauchen der Release-Call und die Vergleichs-Query unten nicht. Wähle ein Ablaufdatum.
3. Kopier den Wert: beginnt mit `fm_` und wird nur einmal angezeigt. Leg ihn im Secret-Store deines CI als `FASTMON_API_KEY` ab.

Eine Pipeline, die nur Releases meldet, kommt ohne die Account-Seite aus: Der **CI/CD**-Schnipsel auf der Application-Seite erstellt direkt dort einen `release:write`-Key für diese Organisation und schreibt ihn in den curl-Befehl.

Key verloren? Widerrufe ihn und erstell einen neuen. Der alte funktioniert sofort nicht mehr. Was ein Key darf und was nicht, steht unter [Authentication](/en/api/authentication) (nur auf Englisch).
