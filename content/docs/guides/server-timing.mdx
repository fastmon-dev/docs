...
**History.** `search_dur` and `kv_dur` exist since 27 August 2026; before that, `redis` counted in `cache_dur` and `elasticsearch` counted nowhere. Since 28 August 2026, `cache_dur` is folded into `kv_dur` and `external_dur` is renamed to `http_dur` (same members). Stored values were not rewritten and `http_dur` was not backfilled, so on a long window `kv_dur` steps up at that date and `http_dur` starts there. `cache_dur` and `external_dur` are gone from the API; the per-key detail before that date is still in `server_timing_dur`. Notification rules that name a retired metric keep evaluating on its successor; new rules take the new names.
