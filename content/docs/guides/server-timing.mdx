...
Fastmon accepts `pageType` / `page_type` and the first-party `fm-pagetype`, lowercases the label, and caps it at 32 chars. The header value takes priority over fastmon's URL-pattern fallback.
