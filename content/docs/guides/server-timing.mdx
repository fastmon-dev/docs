...
The value is uppercased and normalized to a known status — `HIT`, `MISS`, `EXPIRED`, `STALE`, `BYPASS`, `REVALIDATED`, `UPDATING`, `DYNAMIC`, `PASS`, `NONE`. Fastly's compound forms collapse (`MISS-CLUSTER` → `MISS`, `HITPASS` → `PASS`, `HIT-STALE` → `STALE`); anything unknown passes through, capped to 12 chars. The result is the fastest read on whether your edge cache is doing its job, sat right next to [TTFB](/en/web-vitals/ttfb).
