...
- **`checkout_stage` filter.** Cart, checkout, and completed are fixed options in the filter bar, the chips, and the command palette. On the metric pages it is a filter: Web Vitals per funnel stage is a filtered query, not a breakdown.
