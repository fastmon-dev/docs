...
**PII:** the session id is a visitor identifier and its write is the one § 25 (1) TDDDG storage operation the beacon performs, so it is consent-relevant. Leave it off for the leanest posture, or pair it with [`session_consent`](#session-consent) to write only after opt-in. With it off, a row carries no on-device identifier.
...
**PII:** low. Only key names are kept, and they rarely carry visitor-specific data; every value is stripped before the URL leaves the browser. To keep a specific value, you have to opt in explicitly via [`collect_query_values_for`](#collect-query-values-for).
