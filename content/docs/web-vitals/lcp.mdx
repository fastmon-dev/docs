...
LCP is part of [Core Web Vitals](/en/web-vitals), Google's standard user-experience set, and it counts toward search ranking.
...
- **By release**: set `compare_to_release_id` in the [Analytics API](/en/api/analytics/queryAnalytics) to see whether a deploy regressed LCP.
...
For a deeper walkthrough, see web.dev's [Optimize LCP](https://web.dev/articles/optimize-lcp); the underlying optimization advice is the same regardless of which RUM tool you use.
...
- [Interaction to Next Paint (INP)](/en/web-vitals/inp): the responsiveness metric that often regresses when you optimize LCP by adding more JavaScript.
- [Cumulative Layout Shift (CLS)](/en/web-vitals/cls): the visual-stability metric.
- [Web Vitals overview](/en/web-vitals): all five metrics and their thresholds.
