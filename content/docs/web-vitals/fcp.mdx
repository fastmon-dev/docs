...
2. **Reduce render-blocking CSS.** Inline critical CSS for above-the- fold content; load the rest with `media` attributes or `rel="preload" + onload`.
3. **Defer non-critical JavaScript.** A synchronous `<script>` in `<head>` blocks the parser until it executes. Use `defer` or move to `<body>` end.
4. **Optimize web fonts.** Use `font-display: swap` so text renders in the fallback while the web font loads. `font-display: optional` if you can live without the web font on slow connections.
5. **Reduce HTML payload.** A 500 KB HTML document blocks the parser long enough to dominate FCP on slow networks.
