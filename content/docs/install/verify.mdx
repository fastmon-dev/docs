...
- non-zero **page views** and **unique visitors** (the visitor
  count comes from the edge-derived `stitch` and works in every
  mode; only the *session* identity stays at zero in the default
  `anonymous` mode, which is expected. See
...
that the site has **Allowed origins** set and the page you're
loading from isn't on the list. Open the site settings, add the
missing origin (e.g. `staging.acme.com`), and save. The script
cache invalidates automatically. Other silent drops: site marked
inactive, or unknown `collector_hash`.
