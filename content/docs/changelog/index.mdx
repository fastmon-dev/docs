...
The dashboard picks up `ttfb_final` and the two new phases. The response breakdown ends the server phases on the final response headers: where a site answers with 103 Early Hints, the wait carries an "Early Hints" row from the first byte to the final headers, the server phases below it end where that row ends, and the transfer bar starts at the final headers instead of counting the head start a second time as download. A server phase that outlasts the first byte keeps its reported length, drawn in two parts with the seam on the crossing, where it used to be clamped into the wait (a shop with 297 ms of backend was printed as the 61 ms the browser saw). Sites without an interim response, and browsers that do not report the timing, look as before. The TTFB page gets an Early Hints panel for the same fact: the head start at the selected percentile, the two first bytes as one split bar, and the share of pageviews it was measured on; it appears only where the two values differ. Search and KV are phases of their own next to DB, Cache, External, and Render on the Server-Timing page, in the breakdown waterfall, and as Explorer metrics and columns, and the "Backend phases" preset gains a DB-against-search-and-KV chart. The pageview drawer stops adding the backend to its own parts: the composition is a nested list now, each part under what contains it with its share of that row, and unattributed time as its own row. Every phase wears its own color in the breakdown, and the cache phase is called "Cache" rather than "Cache (FPC)", since it sums the application's own cache calls and the full page cache is the origin cache status. See [TTFB → Early Hints](/en/web-vitals/ttfb#early-hints).
...
Country, device, OS, and browser stood in the visitors table only, so a shared `?visitor=` link opened a journey with no idea whose it was. The session header carries them now, in the pinned card and the expanded row, read per pageview because a stitched visitor can cross devices and countries: what varies is listed. The row showed eight of thirty-two hex characters with no way to get the rest; the ID is click-to-copy on hover now, in full, and a row action narrows the page to that visitor: for a stitch a real filter with a chip in the bar, which the other analytics pages read too, and for a session ID, which the backend cannot filter on, the visitor search box above the table. Device, OS, country, and browser each grew the same hover plus and minus the Explorer's pageview table has.
...
`cache_dur` summed `redis` and Shopify's `edge_cart` as if they were cache time, and Elasticsearch counted nowhere, although search carries a large share of backend time in a typical Shopware shop. Two new phases fix that: `search_dur` sums `elasticsearch`, `opensearch`, `solr`, `search`, and `fm-search`; `kv_dur` sums `redis`, `valkey`, `kv`, and `fm-kv`. Both follow the rules of the existing sums (members stay in the maps for drill-down, no source key means `NULL`), are percentile phases on `POST /analytics/server-timing`, row fields on `/analytics/beacons`, and aggregatable metrics. `db_dur` stays purely relational. This changes what `cache_dur` means: `redis` now counts in `kv_dur` (Redis can be sessions, a queue, or the primary store, so its value only says "time in Redis"), and `edge_cart` leaves the sums entirely. Stored values are not rewritten, so a long-window `cache_dur` chart on an affected site steps down at the deploy; a shop that uses Redis as its cache emits `fm-cache` on top. Along the way, two Shopify header keys that are configuration rather than timing (`compressionlevel`, `speculationtags`) are dropped in the collector and in the tracker's own denylist, `compressiontime` joins the catalog and no longer uses one of the eight custom slots, and a numeric description outside the duration window (Shopify's theme id, `theme;desc="190311530496"`) stays a description instead of becoming a six-year duration that skewed every percentile over the key. The [Server-Timing guide](/en/guides/server-timing#backend-phases) has the updated tables.

</ChangelogEntry>

<ChangelogEntry date="2026-08-27" type="new" area="tracker" title="ttfb_final: the real first byte behind 103 Early Hints">

On sites behind 103 Early Hints (Shopify storefronts, Cloudflare) TTFB regularly read below the Server-Timing backend phases of the same pageview. Both were right: `responseStart` points at the first byte of the first response, and with Early Hints that is the interim 103 the edge answers within about one round trip, while the `Server-Timing` header rides on the final response. The tracker now also reads `finalResponseHeadersStart` (wire key `frs`, Chromium 115 and later, `null` elsewhere) and applies the same activation adjustment and 60-second cap as TTFB; the collector stores it as `ttfb_final` next to `ttfb` and drops values below it. Reading the pair: empty means the browser cannot report it or the pageview was a soft navigation or bfcache restore; equal to `ttfb` means no interim response; above `ttfb` means Early Hints are active and the difference is the edge's head start. TTFB itself stays `responseStart`, so it keeps comparing to CrUX. `ttfb_final` is a row field on `POST /analytics/beacons`, part of the visitor pageview detail, and an aggregatable metric. The tracker's Server-Timing denylist also gains Shopify's `compressionlevel` and `speculationtags`, mirroring the collector. See [Server-Timing → Early Hints](/en/web-vitals/ttfb#early-hints).
...
The funnel stage and the cart counters were readable on the Analytics page only. The Explorer now groups by Checkout Stage, in its own group of the Group-by menu: the stage distribution stacked over time, one line per stage for any metric, a stage column in the grouped table, and a row that drills into that stage like any other dimension. The empty stage is a named bucket, "No checkout", because outside the funnel is where most pageviews live. Cart Actions and Failed Cart Actions are metrics, chartable and summable like any other counter, and usable in notification rules; both are derived from fetch/XHR and undercount by construction, so they stay counts beside the funnel and never the denominator of a rate. "Cart Actions at least 1" is a filter, the pageview-level counterpart to the cart toggle on Visitors, and `checkout_stage` can be excluded as well as filtered. The raw pageview table gets stage, cart actions, and failed cart actions as columns, and the pageview drawer a Checkout section with the kinds where the shop system reveals them. A "Checkout" preset brings stages over time, LCP per stage, and cart actions against failed ones, scoped to the three funnel stages; leaving a preset clears the scope it brought. The checkout panel on the dashboard links into the Explorer with the same window and site scope. On the API, `checkout_stage` is a group-by dimension and an exclude in `/analytics/query`, `cart_interactions` and `cart_interaction_errors` are metrics and metric ranges there, and `/analytics/beacons` projects all of them onto each row. Queries on these read the pageview source; the aggregate tiers are untouched.
...
Shops could see how fast their pages are, but not whether visitors get through to the checkout. Now every pageview whose URL is a cart, checkout, or thank-you page carries that stage, stamped at ingest against built-in patterns for Shopware 5 and 6, WooCommerce, Magento 2, PrestaShop, JTL 5, and Shopify (language prefixes like `/de/warenkorb` included). A first-party `POST` to a known cart endpoint additionally marks the pageview as a cart interaction, read from the fetch/XHR telemetry the tracker already sends. Nothing changes in the tracker and nothing new is collected: both are labels over data the site already stores, and everything per visitor is computed at query time. On the dashboard, Checkouts becomes the fourth headline metric, with cart and checkout drawn as context lines in the chart when it is selected; a checkout panel lists visitors per stage and cart requests by kind (it replaces Tech insights); the current-visitors card carries the three stages; and the count tiles show a per-minute average so a day and a month compare. `checkout_stage` is a filter dimension everywhere, the Visitors list filters to everyone who reached a stage or touched the cart and shows both on the journey, and Business Impact reports the measured conversion rate where the funnel has data. Every number is a count of something observed. There is deliberately no abandonment metric, because "abandoned" would be a claim built from a missing observation; the success step counts thank-you pages, not payments. The feature is on by default, with an opt-out and a per-stage path editor under Settings → Checkout funnel. Counting per stage needs a stored visitor id (`store_stitch`), and cart interactions need `collect_fetch_xhr`. On the API: `POST /analytics/funnels/checkout` and `/journeys`, plus `contains_checkout_stage` and `contains_cart_interaction` on the visitors list. The [Checkout funnel](/en/checkout-funnel) page has the patterns, the OXID recipe, and the caveats.

</ChangelogEntry>

<ChangelogEntry date="2026-08-26" type="new" area="dashboard" title="Switch a filter off without deleting it">

Answering "what does this look like without the country filter" meant deleting the filter and typing it back in afterwards. Every filter chip now has an on/off switch: switched off, the chip keeps its values and stays where it is, drawn struck through, and the query runs without it. This works the same for include filters, excludes, metric ranges, and the boolean filters. A switched-off filter narrows nothing, and the rest of the page agrees: it is left out of the badge count, the map draws no country as picked, the device segments read as unpicked, and clicking any of them switches the filter back on rather than adding a second one. The state rides in the existing URL parameters as a leading `!` on the entry's key, so a link shared before this existed opens with everything on. The scope line also shows a country by its flag instead of its two-letter code.
...
The summary above the Visitors list computed average duration and average pages per visitor over the rows currently loaded, so both moved when you paged forward, and the default "at least 3 pages" filter put a floor under one of them. It now describes the selected period: visitors, visitors with a single page, and the median pages per visitor. Average duration is gone rather than repaired, because there is no window-wide visit duration to compute it from. Country shows its flag and localized name in the table and the cards, with the code on hover. In the URL, behavior filters are plain numbers (`?minPages=3`), and the deep-linked visitor parameter is `visitor` instead of `session`, since it holds a stitch or a session id depending on the identity; old links keep working and rewrite themselves on first use. Two fixes ride along: an old `/sessions` link carrying a page-type filter arrived unfiltered, and a 32-digit visitor id that reached the app as a number was passed on with its digits already lost. The redirect forwards its filters now, and an id that does not survive the round trip exactly is dropped instead of corrupted.

</ChangelogEntry>

<ChangelogEntry date="2026-08-25" type="new" area="dashboard" title="Response breakdown: where a typical response spends its time">

The Network section gets a new full-width card, the response breakdown. It replaces the Network Request Timing chart, which showed the same four metrics as one bar with nothing to drill into. The new card draws a waterfall at the selected percentile, the way browser devtools do: DNS, connect, waiting for the first byte, and transfer as a staircase, followed by the LCP phases (load delay, load duration, render delay) as their own group. Server-Timing phases (origin, edge, backend) nest inside the wait as the server's share, and the CDN and origin-cache comparisons split their bars into hit and rest. FCP, LCP, and DOM Interactive sit as markers on the axis; a late marker such as Load pins to the right edge as a value pill instead of stretching the scale. Percentiles of individual phases do not add up, so the card derives the wait as the remainder and scales the LCP phases to end exactly at the LCP marker; every tooltip shows the raw reported value, its position on the timeline, and the share of requests that reported the phase. Below the waterfall, rows list page types (where your site reports them) or pages, each with a compact bar in the same colors. Clicking a row scopes the waterfall to it, shown as an "Overall › cart" breadcrumb, and the selection is part of the URL, so a shared link opens the same scoped view. The pageview drawer in the Explorer shows the same waterfall in small, built from that one pageview's metrics. One fix rides along: a deep link to a card further down the page (`?panel=`) did not scroll, because that section had not been rendered yet. The linked section now loads immediately, which also repairs existing share links to the page table.

</ChangelogEntry>

<ChangelogEntry date="2026-08-24" type="new" area="dashboard" title="Top pages and page types in one performance table">

The Top Pages card under Traffic & Visitors listed views and LCP, and page type performance had no place at all, even though a slow checkout hides behind a fast home page in site-wide numbers. The card becomes a full-width table: views with a share-of-traffic bar, then LCP, INP, CLS, and TTFB side by side, each a status-colored pill at the chosen percentile. A toggle in the header switches between page types and pages. Sites with page type detection open on page types, with untyped traffic as a muted row at the end so the shares add up; sites without detection land on pages, and the page types view points to the application settings for setting detection up. Rows still filter the dashboard or drill into the Explorer. The traffic trend gets a full row of its own again: trend and table stack instead of sharing a 2/3 + 1/3 row. Large counts abbreviate the same way everywhere now: Browsers, Bots & visitors, and Devices use the same K/M formatting as the table, with the exact number in a tooltip.

</ChangelogEntry>

<ChangelogEntry date="2026-08-06" type="new" area="platform" title="First-party proxies can forward the real visitor IP">

Relaying beacons through your own server (a first-party proxy) used to collapse every visitor into one: from our edge's view all requests came from your proxy's IP, and honoring an `X-Forwarded-For` from an arbitrary sender would open the door to spoofing. There is now an explicit grant per application: generate a proxy secret under **Settings → Collector**, send it in the `FM-Proxy-Key` header on the beacon route, and the edge resolves the real visitor IP from your proxy's `X-Forwarded-For`. Geo and visitor counts then work as if the beacon had arrived directly. Rotating creates a new secret while the previous one stays valid until the next rotation, so a proxy fleet can be reconfigured without a gap, and deleting the secret switches the trust off. On the API this is `POST` and `DELETE /v1/applications/{id}/proxy-secret`. The [first-party proxy guide](/en/guides/first-party-proxy) has ready-made nginx, Caddy, and Cloudflare Worker snippets.
...
Two changes to the `shopware6` ruleset. First, the storefront home page counted as `category`: Shopware 6 renders it through the same navigation controller as category pages, and the `is-ctl-index` class the mapping expected does not exist there. The tracker now reads the controller and action class together (`is-ctl-navigation is-act-home`), so home pages count as `home`. Second, Shopware 6.8 removes the `is-ctl-*` and `is-act-*` body classes entirely, so the tracker now prefers their designated replacement, the `is-active-route-*` class that shops emit since about 6.6. Detection keeps working across the 6.8 upgrade with no action on your side, and on the route class the checkout splits properly: the cart page counts as `cart`, the confirm, finish, and register steps as `checkout`, and the whole account area (orders, profile, addresses) as `account`. Shops without the route class fall back to the legacy classes, where cart and checkout still share the coarser `cart`. A browser-cached tracker keeps its old behavior until the cache rolls over. The [ruleset table](/en/tracker#ruleset-page-types) shows the updated mapping.
...
A round of filter work. Clicking a country on the world map now works and opens a popover with the country's numbers and the + and - actions (filter to it or exclude it, also reachable from the hover tooltip); excluded countries get a red outline, and the map keeps zoom and pan during a refetch instead of resetting. The filter chips in the header get quick actions: the chip opens the panel straight into its value editor, an x removes the filter, and hovering lists every selected value with its own x. In the value editor, your selected values are pinned at the top and stay removable even when they match no traffic (an excluded country used to disappear from its own list), and including and excluding the same value can no longer coexist. And filtering no longer shakes the dashboard: the performance tiles, the geo map, and the Core Web Vitals card keep showing the previous values while the new ones load, instead of swapping to a full-height spinner.

</ChangelogEntry>

<ChangelogEntry date="2026-08-04" type="new" area="dashboard" title="The Explorer table sorts on the server">

Clicking a column header now re-queries with that order, and the backend sorts before it truncates to the row limit, so "the ten worst TTFB pages" really are the worst across all matching traffic. Previously a header click only reordered the rows the browser had already loaded, which meant "the worst among the busiest". The sort travels in the URL, so a shared link opens in the same order. Nothing sorts implicitly anymore: selecting a metric adds a column and nothing else, and without a chosen sort the table states the backend's default order instead of pretending one. The raw pageview list sorts server-side too, by time or by any row column, with empty values sinking to the end in either direction; its visible columns are part of the link as well (`?cols=`), so a bookmark reopens the table exactly as it was. The metric columns also moved out of the buried "Columns" menu up to the query panel, next to aggregation and grouping. On the API, `/analytics/beacons` accepts `order_by` as a `{field, direction}` object over any scalar row field.
...
An origin can send a Server-Timing key as a label rather than a timing, for example `pop;desc="DE-1331"` for the CDN node that served the request. Those keys were stored but invisible, because everything ranked by milliseconds. The Server-Timing page now lists them below the timed keys with the mix of their values, a pageview's drawer shows them as plain key/value fields, the Explorer can show any custom key as a column and sort and filter on it, and the pageview list can be scoped to a key or one of its values (`/analytics/beacons` gains a `server_timing` filter for this). The common PoP keys (`pop`, `fastly-pop`, `cdn-pop`, `x-qc-pop`) are no longer dropped at ingest: they name the CDN node, server infrastructure rather than the visitor (`asn`, `country`, and `ipv6` stay blocked). Also fixed along the way: expanding the trend of a custom key whose name contains a hyphen or a dot (`wp-total`, `cdn-upstream-fbl`) returned an error.
...
The Explorer had grown a filter apparatus of its own: its own Filter button, a fold-out strip with two columns and two add buttons, right next to the standard filter menu that does the same job everywhere else. The standard controls take over now: query filters live in the scope menu in the header, presets share one menu with your saved queries, and the client-side result filters sit above the table they narrow down. Refresh and live mode move from the far end of the header into the time-range control, since they act on that same window. And the filter count now counts every kind of filter: an exclude, a metric range like `lcp >= 2.5s`, or the bot switch used to be applied while the button still read as nothing filtered.
...
A round of interface work across the signed-in app, focused on mobile and on filtering. The dashboard header is rebuilt into two halves, where you are on the left and what you're filtering on the right, and the time range, percentile, device, and filters fold into one scope summary (`Last 7 days · p75 · 📱 · Country: DE +4`) with the full controls in a popover behind it; active filters now sort by importance, so the same selection always looks the same. On a phone the sidebar becomes an off-canvas drawer that slides the content aside instead of covering it, dialogs stay within the screen (long values truncate instead of pushing the buttons off-screen), and headings and spacing are consistent across pages. The filter editor also gets three fixes: editing a single-valued filter shows its current value, the value counts respect the filters you already applied, and the "Hide bots" switch no longer appears while you edit an unrelated filter. And the sidebar's documentation, feedback, and support links collapse into one Help button.
...
Every request now carries a traffic verdict: **Human**, **Likely bot**, or **Bot**. *Bot* is a declared or verified crawler (a bot user-agent, or an IP in a published crawler range); *Likely bot* is a datacenter or VPN address, or a user-agent anomaly, with no bot declaration (so a visitor on a VPN lands here too); *Human* is everything else. A **Hide bots** checkbox drops the whole view to real visitors in one click, and a new **Bots & visitors** chart splits traffic by verdict and lists the top bots by name and the bot types. New filters cover the datacenter or hosting network, the bot name, the bot type, and the user-agent anomaly. Detection runs at our edge from the IP and user-agent against a small fixed list of known providers and crawlers; the IP and user-agent are discarded right after, so nothing raw is stored (see [what we keep and why](/en/privacy#bot-and-datacenter-labels)).
