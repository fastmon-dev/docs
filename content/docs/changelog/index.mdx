...
Single-page route changes (soft navigations) used to record no loading metrics, so LCP and FCP for SPA-heavy sites came only from cold page loads and read worse than the real experience. Soft navigations now report **Route Load** (`soft_nav_load`), a network-based proxy derived from the route's own same-site requests, on every browser and surfaced as its own metric and alert (labeled a proxy, never merged into LCP or FCP). Real LCP and FCP for soft navigations also arrive on Chromium once its Soft Navigations API ships; expect SPA LCP/FCP p75 to start reflecting in-app navigations then, a methodology change rather than a regression.
