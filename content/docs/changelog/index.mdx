...
<ChangelogEntry date="2026-07-10" type="new" area="tracker" title="Runtime tuning knobs for the beacon">

For pages that need finer control over how many beacons they send, the tracker now reads a few `window` globals: `__FASTMON_SAMPLE_RATE` (sample a fraction of page-views), `__FASTMON_INIT_COALESCE_MS` (merge the first two beacons on a fast load into one request), `__FASTMON_UPDATE_MIN_AGE_MS` (hold early mid-page-view updates for the final beacon), and `__FASTMON_MAX_VERSION` (cap update beacons so a polling page can't fire thousands). The defaults are sensible, so most sites never touch them. See [The RUM Beacon](/en/concepts/beacon#tuning-knobs).

</ChangelogEntry>

<ChangelogEntry date="2026-07-09" type="changed" area="platform" title="Collection modes are now presets and per-setting configuration">
...
<ChangelogEntry date="2026-07-09" type="changed" area="tracker" title="Per-domain visitor signal, and no cookies">

The edge-derived `stitch` signal now includes the page domain in its input, so a visitor gets a different signal on each domain: an org-level tracker routes to a site per domain but no longer counts a visitor as one across domains. The beacon also sets no cookies in any build; the only thing written to the device is a single `sessionStorage` entry (`_fms`), the session id, and only when `store_session` is on. Your consent-management platform switches that write on with `window.fastmon.grantConsent()`. See [The RUM Beacon](/en/concepts/beacon) and [Privacy](/en/privacy).

</ChangelogEntry>

<ChangelogEntry date="2026-07-09" type="changed" area="dashboard" title="Visitors Explorer, with stitch as a first-class identity">
...
<ChangelogEntry date="2026-07-09" type="changed" area="dashboard" title="Cross-domain trackers, and one-click site provisioning">
...
<ChangelogEntry date="2026-07-09" type="fixed" area="platform" title="One bad value no longer drops a batch of beacons">
