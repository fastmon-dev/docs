...
<ChangelogEntry date="2026-07-27" type="changed" area="platform" title="Creating an organization now asks for your website">

Creating an organization now asks for the domain of the website you want to measure. You can paste a full URL (`https://www.Example.com/shop`) and fastmon reduces it to the hostname (`example.com`). It's the main thing an organization's approval is decided on. Approval still gates collection: a new organization shows a "we're reviewing your organization" note until it's approved, and an organization that gets blocked sees a banner that says collection is paused and names the support address, so a wrong call is always appealable. Approvals and blocks are emailed to the organization's owners in their account language.
...
<ChangelogEntry date="2026-07-26" type="changed" area="platform" title="Marketing and legal pages moved to fastmon.eu">

The app is now only the app. The marketing landing page is gone from it: signed-out visitors go straight to login, signed-in ones to the dashboard. The Terms, Privacy, Legal notice/Impressum, and DPA pages now live on the corporate site, and the old in-app `/terms`, `/privacy`, `/legal-notice`, and `/avv` links redirect there, so existing bookmarks keep working. The app itself is now marked `noindex`.
