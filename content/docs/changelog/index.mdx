...
A minute-granularity view used to be capped at a 2-hour window; it now accepts up to 6 hours, so you can see a full 6-hour span at minute resolution in one go. Longer spans still fall back to hourly or daily buckets.

</ChangelogEntry>

<ChangelogEntry date="2026-07-10" type="new" area="dashboard" title="Cache status in the per-visitor journey">

The per-visitor journey in the Visitors Explorer now shows the CDN and Origin cache result (HIT / MISS) for each page-view, so you can see whether a slow page for one visitor was served warm or cold. A page-view with no captured cache status simply omits the badge.
