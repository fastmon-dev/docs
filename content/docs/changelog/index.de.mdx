...
Single-Page-Routenwechsel (Soft-Navigationen) erfassten bisher keine Loading-Metriken, LCP und FCP für SPA-lastige Sites kamen also nur aus kalten Seitenladungen und lasen sich schlechter als die echte Erfahrung. Soft-Navigationen melden jetzt **Route Load** (`soft_nav_load`), einen netzwerkbasierten Proxy aus den Same-Site-Requests der Route, in jedem Browser und als eigene Metrik plus Alert (als Proxy gekennzeichnet, nie in LCP oder FCP verrechnet). Echte LCP und FCP für Soft-Navigationen kommen zusätzlich auf Chromium, sobald dessen Soft-Navigations-API ausgeliefert wird; dann spiegelt der SPA-LCP/FCP-p75 auch In-App-Navigationen wider, eine Änderung der Messmethodik, keine Regression.
