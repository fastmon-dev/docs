...
<ChangelogEntry date="2026-07-10" type="new" area="tracker" title="Runtime-Tuning-Knöpfe für den Beacon">

Für Seiten, die feiner steuern wollen, wie viele Beacons sie senden, liest der Tracker jetzt ein paar `window`-Globals: `__FASTMON_SAMPLE_RATE` (nur einen Anteil der Pageviews sampeln), `__FASTMON_INIT_COALESCE_MS` (die ersten zwei Beacons bei schnellem Load zu einem Request zusammenfassen), `__FASTMON_UPDATE_MIN_AGE_MS` (frühe Mid-Pageview-Updates für das finale Beacon zurückhalten) und `__FASTMON_MAX_VERSION` (Update-Beacons deckeln, damit eine pollende Seite nicht Tausende feuert). Die Defaults sind sinnvoll gewählt, die meisten Sites fassen sie also nie an. Siehe [Der RUM-Beacon](/de/concepts/beacon#tuning-knobs).
...
<ChangelogEntry date="2026-07-09" type="changed" area="tracker" title="Besuchersignal jetzt pro Domain, und keine Cookies">

Der am Edge abgeleitete `stitch` enthält jetzt die Domain im Input, ein Besucher bekommt also auf jeder Domain ein anderes Signal: ein org-weiter Tracker routet auf eine Site pro Domain, zählt einen Besucher aber nicht mehr domainübergreifend als einen. Der Beacon setzt außerdem in keinem Build ein Cookie; das Einzige, was auf das Gerät geschrieben wird, ist ein einzelner `sessionStorage`-Eintrag (`_fms`), die Session-ID, und nur wenn `store_session` an ist. Deine Consent-Management-Plattform schaltet diesen Schreibvorgang mit `window.fastmon.grantConsent()` frei. Siehe [Der RUM-Beacon](/de/concepts/beacon) und [Privacy](/de/privacy).
