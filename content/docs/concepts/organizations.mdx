...
The bundles nest: a member holds everything a viewer holds, an owner everything a member holds. [API keys](/en/api/authentication) are cut from the same bundles: a key's scopes are permissions, and it can never do more than the role of the user who created it.
