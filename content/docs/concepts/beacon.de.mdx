...
- DOM-Inhalt anfassen (kein Scroll-Replay, kein Session-Replay, keine Formularerfassung). Die einzige Ausnahme ist der Opt-in-Page-Type-Read ([`pagetype_ruleset`](/de/tracker#pagetype-ruleset), oder die Klassennamen, die du selbst in [`pagetype_rules`](/de/tracker#pagetype-rules) auflistest), der einen einzelnen Body-Class-Token zur Page-Type-Einordnung nimmt, sonst nichts.
...
- Metrik-Keys sind kurze Mnemonics: `ttfb`, `fcp`, `lcp`, `inp`, `cls`, Page Load (`plt`), Cache-Hit-Rate (`chr`), die Resource- und Third-Party-Rollups je Typ sowie die Long-Animation-Frame-Aggregate je Skript. `frs` ist `finalResponseHeadersStart` (gespeichert als `ttfb_final`, außerhalb von Chromium `null`), das erste Byte der finalen Response, wo 103 Early Hints `ttfb` auf die vorläufige zeigen lassen. Früher waren es undurchsichtige nummerierte Slots (`t1`…`t36`); gecachte ältere Bundles senden diese Namen weiterhin, und der Collector bildet sie auf dieselben Felder ab.
- `errc`, `frc`: die beiden Fehlerzähler, getrennt gehalten. `errc` zählt JavaScript-Fehler (Exceptions und unbehandelte Rejections), `frc` zählt fehlgeschlagene `<img>`/`<script>`/`<link>`-Ladevorgänge. Beide erfassen jedes Vorkommen, nicht nur die Einträge, die unten Platz finden, und beide fehlen (statt auf 0 zu stehen), wenn das Beacon für ihren Kanal nichts gemeldet hat: der Pixel-Fallback und der Pfad für übergroße Payloads senden weniger Felder als ein normales Beacon. Siehe [was als Fehler zählt](/de/tracker#what-counts).
- `errs`: Error-Einträge (`t` Typ, `n` Count, `m` Message, `fr` Frames), bis zu acht: die fünf häufigsten JavaScript-Fehler, danach die drei häufigsten fehlgeschlagenen Ressourcen, jeweils innerhalb des eigenen Kanals sortiert. `t` und `n` gehen in jeder Konfiguration raus, ebenso der Host jedes Frames; dazu Pfad und Zeile/Spalte der Frames in deinen eigenen First-Party-External-Scripts. Die Message `m` ist Opt-in über [`collect_error_messages`](/de/tracker#collect-error-messages), Pfad/Zeile/Spalte von Inline- und Third-Party-Frames über [`collect_error_frames`](/de/tracker#collect-error-frames); auch dann läuft `m` durch einen Scrubber, der URLs, E-Mails, IDs und Strings in Anführungszeichen maskiert. Die **Fehler-Herkunft** eines Pageviews wird serverseitig aus den angekommenen Einträgen abgeleitet: `first`, wenn ein JavaScript-Eintrag einen Frame auf der eigenen registrierbaren Domain der Seite hat, `third`, wenn keiner das hat und ein fremder Host gesehen wurde, und leer, wenn es keinen Fehler oder keinen vergleichbaren Host gab.
...
- Fetch/XHR und `Server-Timing`: aggregierte Fetch/XHR-Werte je Endpunkt (Aufrufzahlen, Dauern, Dauer-Buckets, Statusfehler) und die `Server-Timing`-Metriken der Seite. Beide sind templated und werden serverseitig normalisiert (die Leitung ist fälschbar), also tragen Pfade keine Query-Strings oder rohen IDs. Zu den promoteten `Server-Timing`-Feldern gehören die beiden Cache-Alter, `cdn_age` und `origin_age` in Sekunden, neben den Cache-Status, die sie qualifizieren; siehe [Server-Timing](/de/guides/server-timing#cache-age).
