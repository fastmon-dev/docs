...
Die Liste filtert danach, was die Journey eines Besuchers enthält: `contains_page_type` für einen Page-Type, bei Shops `contains_checkout_stage` (`cart`, `checkout` oder `success`) und `contains_cart_interaction` für den [Checkout-Funnel](/de/checkout-funnel). Jeder Pageview im Detail trägt seine `checkout_stage` und die Warenkorb-Interaktionsfelder. Der Funnel selbst hat eigene Endpunkte unter `/analytics/funnels/checkout`, gruppiert über dieselbe Identität.
