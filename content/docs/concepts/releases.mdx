...
Performance regressions almost always line up with a deploy. Without release markers, you've got to remember when each deploy happened and compare by hand. With markers, the [Analytics API](/en/api/analytics/queryAnalytics) does the before/after comparison for you in a single request.

## When you'd touch it

- **Every deploy.** Cleanest setup: a `POST /sites/{id}/releases` from CI/CD after a successful deploy.
- **Investigating a regression.** Use the release picker in the dashboard to overlay metrics across releases.
...
Use whatever your CI considers a deploy version: git SHA, tag, build number. Whatever you pick, stick with it. Mixing styles makes the sort order meaningless.
...
- **`released_at` matters more than `id`.** Sorting is by `released_at`. Back-date a release and the order will look wrong.
- **Releases are per site.** A monorepo shipping several sites needs one release call per site, even if the version string is identical.
- **Deleting a release breaks `compare_to_release_id` requests that reference it.** The endpoint returns `404`. Metrics already rendered aren't affected.

## Related

- [Sites](/en/concepts/sites): releases live under a site.
- [Analytics API](/en/api/analytics/queryAnalytics): `compare_to_release_id`.
- [Compare releases](/en/guides/compare-releases): step-by-step guide for hooking up CI/CD.
